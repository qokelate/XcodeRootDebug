...
#include <unistd.h>
#include "MobileSubstrate/usr/include/substrate.h"
...
bool hooked_SMJobSubmit(CFStringRef domain, CFDictionaryRef job, AuthorizationRef auth, CFErrorRef _Nullable *error) {
	LOG(@"Enter hooked_SMJobSubmit %@, enabled: %d, debugserver: %@", job, enabled, debugserverPath);
...
static void *(*pMSFindSymbol)(MSImageRef image, const char *name) = NULL;
static MSImageRef (*pMSGetImageByName)(const char *file) = NULL;
static void (*pMSHookFunction)(void *symbol, void *replace, void **result) = NULL;

%ctor {
  void *sub = dlopen("/var/jb/usr/lib/libsubstrate.dylib", RTLD_NOW);

  pMSFindSymbol = dlsym(sub, "MSFindSymbol");
  if (NULL == pMSFindSymbol) pMSFindSymbol = dlsym(sub, "_MSFindSymbol");

  pMSGetImageByName = dlsym(sub, "MSGetImageByName");
  if (NULL == pMSGetImageByName) pMSGetImageByName = dlsym(sub, "_MSGetImageByName");

  pMSHookFunction = dlsym(sub, "MSHookFunction");
  if (NULL == pMSHookFunction) pMSHookFunction = dlsym(sub, "_MSHookFunction");
  ...
  MSImageRef image = pMSGetImageByName("/System/Library/PrivateFrameworks/ServiceManagement.framework/ServiceManagement");
  if (!image) {
    LOG("ServiceManagement framework not found, it is impossible");
    return;
  }
  pMSHookFunction(
    pMSFindSymbol(image, "_SMJobSubmit"),
